...
namespace FoodEcomerce.Reposiroty.Products
{
    public class ProductRepository : BaseRepository<Product, ProductModal, ProductDTO, Guid>, IProductRepository
    {
        ...
        public async Task<List<sp_WebFood_GetAllProduct>> GetAll(int pageNumber, int pageSize, string ids , int orderType = 3)
        {
            return await _storeContext.sp_WebFood_GetAllProduct.FromSql($"Execute sp_WebFood_GetAllProduct @pageNumber={pageNumber} , @pageSize={pageSize} , @categoryIds={ids} , @orderType={orderType}").ToListAsync();
        }
